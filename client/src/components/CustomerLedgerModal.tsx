...
import { X, CreditCard, ChevronLeft, ChevronRight, User, Phone, Printer } from 'lucide-react';
  ...
  // print function
    ...
    try {
      ...
      if (totalPages > 1) {
        const pages = await Promise.all(
          Array.from({ length: totalPages }, (_, i) =>
            customersApi.getAccount(customerId, i + 1).then(r => r.data)
          )
        );
      ...
      printAccountStatementA4({
        title: 'كشف حساب عميل',
        subjectName: customer?.name ?? customerName,
        generatedAt: fmtDate(new Date().toISOString()),
        printedRowsCount: allTransactions.length,
        cards: [
          { label: 'العميل', value: customer?.name ?? customerName },
        ...
        rowsHtml: rows,
        footerText: 'تم إنشاء هذه الطباعة من كشف الحساب الكامل للعميل.',
      });
    } finally {
      setIsPrinting(false);
    }
